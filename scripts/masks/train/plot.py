...
def plot_sst_attributions(attributions, tokens, name=None, save=False):
    ...
    to_plot = (attributions / attributions.abs().max(0, keepdim=True).values)
    average = torch.mean(to_plot, 1).tolist()
    last_cl = to_plot[..., -1].tolist()
    min_val = to_plot[..., -1].min()
    scaled = (to_plot[..., -1] - min_val) / (to_plot[..., -1] - min_val).max()

    tokens = [f"{token}" for token, avg, last, scl in zip(tokens, average, last_cl, scaled.tolist())]
    ...
    plt.xticks(torch.arange(0, 7, 3) + 0.5, ["E"] + list(range(3, 7, 3)), size=16)
